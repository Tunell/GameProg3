...
namespace Engine{
		...
		Ball::Ball(int x,int y, int w, int h, int xSpeed, int ySpeed, Image bild): Komponent(x,y,w,h,bild){
			xx = x;
			yy = y;
			xVel = xSpeed;
			yVel = ySpeed;
		...
		void Ball::handleCollision(Komponent* other){
			...
			if(Player* playerElement = dynamic_cast<Player*>(other)){
				if(playerElement->getPlayerNumber()==1)
					if( xVel < 0 )
						xVel=-xVel;
				if(playerElement->getPlayerNumber()==2)
					if( xVel > 0 )
						xVel=-xVel;
			}
		}
